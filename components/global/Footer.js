import { footerPaymentIcons, footerQuickLinks, footerSocialIcons, footerUseFullLinks } from "@/constants/constant";
...
export default function Footer() {
    ...
    const footerContent = {
        ...
        footer_text2: 'Upload, manage, and access your files with full control and end-to-end protection.',
        ...
        <div className="footerWrapper mx-auto max-w-7xl items-center justify-between mt-5 mb-5 md:px-6 px-4 lg:px-4 xl:px-0">
            <div className="border-1 border-[#43E1A9] footerInnerWrap grid grid-cols-12 gap-3 bg-[url(/images/footer-bg.png)] bg-cover bg-no-repeat rounded-2xl xl:p-8 lg:p-8 md:p-8 p-3">
                ...
                <div className="col-span-6 xl:col-span-2 lg:col-span-2 xl:text-center lg:text-center md:text-left text-center xl:mt-0 lg:mt-0 md:mt-5">
                    <p className="text-[#43E1A9] xl:text-lg text-lg lg:text-[1rem]">{footerContent.quick_links_text}</p>
                    <div className="footerLinks flex flex-col xl:items-center lg:items-center md:items-start items-center gap-3 mt-8">
                        {
                            footerQuickLinks.map((link) => (
                                <Link href={link.link} key={link.name} className="text-center rounded-full border-1 border-[#43E1A9] max-w-[130px] min-w-[130px] text-white py-2 xl:text-[1rem] lg:text-[0.900rem] md:text-[0.800rem] text-[0.750rem]">{link.name}</Link>
                            ))
                        }
                    </div>
                </div>
                <div className="col-span-6 xl:col-span-2 lg:col-span-2 xl:text-center lg:text-center md:text-left text-center xl:mt-0 lg:mt-0 md:mt-5">
                    <p className="text-[#43E1A9] xl:text-lg text-lg lg:text-[1rem]">{footerContent.usefullInfoText}</p>
                    <div className="footerLinks flex flex-col xl:items-center lg:items-center md:items-start items-center gap-3 mt-8">
                        {
                ...
                <div className="w-full col-span-12 lg:col-span-4 xl:col-span-4 xl:pl-5 lg:pl-5">
                    <h4 className="text-[#FDFDFD] xl:text-[1.25rem] lg:text-[1.25rem] md:text-[1.25rem] text-[1.25rem] xl:mt-0 lg:mt-0 mt-5">{footerContent.StayTuned}</h4>
                    <p className="text-[#FDFDFD] xl:text-lg lg:text-lg xl:w-[60%] lg:w-[80%] xl:mt-5 lg:mt-5 md:mt-5 mt-5">{footerContent.stay_tuned_text}</p>
                    <div className="inputField xl:mt-8 lg:mt-8 mt-8">
                        <input type="email" placeholder={footerContent.email_placeholder} className="outline-0 rounded-full p-3 border-1 border-[#43E1A9] text-white/30 xl:text-[1rem] lg:text-[1rem] md:text-[1rem] xl:w-[75%] lg:w-[80%] md:w-[60%] w-[88%]" />
                        <span className="cursor-pointer bg-[url(/images/global/btn-bg.png)] bg-cover bg-no-repeat rounded-full xl:p-[3.8%] lg:p-[4.8%] md:p-[2.2%] p-[4.7%] text-[#FDFDFD] xl:ml-[-20%] lg:ml-[-20%] md:ml-[-10%] ml-[-20%] xl:text-[1rem] lg:text-[1rem] text-[1rem] border-1 border-[#43E1A9] backdrop-blur-[20px]">{footerContent.emailButton}</span>
                    </div>
                    <span className="copyRight text-[#39FF14] xl:text-sm xl:w-[82%] lg:w-[95%] w-[98%] text-right block xl:mt-10 lg:mt-10 mt-10">{footerContent.copyright}</span>
                </div>
            </div>
        </div>
    );
